...
<body>
    <div class="container">
        <header><h1>{self.logo} {self.title}</h1><p class="subtitle">Generated: {datetime.now().strftime("%B %d, %Y at %I:%M %p")}</p></header>
        <div class="stats-grid">{summary_cards_html}</div>
        {group_summary_html}
        <div class="controls">
        ...
        <div class="table-container">
            <table id="dataTable"><thead><tr>{column_headers_html}</tr></thead><tbody>{table_rows_html}</tbody></table>
        </div>
        <div class="info-bar"><span id="recordCount">{len(data)}</span> records shown. Click headers to sort.</div>
    </div>
    <script>
        ...
        function searchTable() {{
            ...
            initialRows.forEach((rowObj, index) => {{
                const tr = tbody.querySelector(`tr[data-index='${{index}}']`) || document.createElement('tr'); // Create if not exists
                if (!tbody.contains(tr)) {{ tr.innerHTML = rowObj.html; tr.dataset.index = index; }} // Ensure it has content

                const isVisible = filter === '' || rowObj.data.some(text => text.toUpperCase().includes(filter));
                tr.classList.toggle('hidden', !isVisible);
                if (isVisible) {{
                    visibleCount++;
                    if (filter) {{ // Apply highlighting
            ...
            if (filter === '') {{ // If search is cleared, re-render all rows to remove highlights
        ...
        document.addEventListener('DOMContentLoaded', () => {{
            // Store original row HTML and data for searching and sorting
